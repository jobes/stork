...
FFI_EXPORT void
stork_canard_register_log_callback(StorkCanardLogCallback callback) {
...
typedef void (*StorkCanardTransferCallback)(
    uint64_t timestamp_usec, uint16_t data_type_id, uint8_t transfer_type,
    uint8_t source_node_id, uint8_t transfer_id, uint8_t priority,
    const uint8_t *payload, uint16_t payload_len);

static StorkCanardTransferCallback g_transfer_cb = NULL;

FFI_EXPORT void
stork_canard_register_transfer_callback(StorkCanardTransferCallback cb) {
  g_transfer_cb = cb;
}

typedef uint8_t (*StorkCanardShouldAcceptCallback)(
    uint16_t data_type_id, uint8_t transfer_type, uint8_t source_node_id,
    uint64_t *out_data_type_signature);

static StorkCanardShouldAcceptCallback g_accept_cb = NULL;

FFI_EXPORT void
stork_canard_register_accept_callback(StorkCanardShouldAcceptCallback cb) {
  g_accept_cb = cb;
}
...
void onTransferReception(CanardInstance *ins, CanardRxTransfer *transfer) {
  if (g_transfer_cb == NULL)
    return;

  uint16_t payload_len = transfer->payload_len;
  if (payload_len == 0) {
    g_transfer_cb(transfer->timestamp_usec, transfer->data_type_id,
                  transfer->transfer_type, transfer->source_node_id,
                  transfer->transfer_id, transfer->priority, NULL, 0);
    return;
  }
  ...
  if (payload_len <= MAX_PAYLOAD) {
    ...
    // Drop payloads that are unreasonably large to prevent stack or heap
    // overflow issues
    if (payload_len > 8192) {
      stork_log("stork_canard: Payload length %d exceeds safety limit, "
                "dropping transfer.\n",
                payload_len);
      return;
    }
    ...
    if (payload_ptr == NULL) {
      stork_log("stork_canard: Failed to allocate %d bytes for payload on "
                "heap, dropping transfer.\n",
                payload_len);
      return;
    }
  ...
  // Optimization: Most messages (e.g. NodeStatus) are single-frame, where the
  ...
  g_transfer_cb(transfer->timestamp_usec, transfer->data_type_id,
                transfer->transfer_type, transfer->source_node_id,
                transfer->transfer_id, transfer->priority, payload_ptr,
                payload_len);
...
bool shouldAcceptTransfer(const CanardInstance *ins,
  ...
  if (g_accept_cb != NULL) {
    return g_accept_cb(data_type_id, transfer_type, source_node_id,
...
FFI_EXPORT void stork_canard_init(uint8_t node_id) {
  canardInit(&g_canard, g_arena, sizeof(g_arena), onTransferReception,
...
FFI_EXPORT void stork_canard_get_stats(uint16_t *capacity, uint16_t *usage,
                                       uint16_t *peak, uint32_t *seq_errors) {
  CanardPoolAllocatorStatistics stats =
      canardGetPoolAllocatorStatistics(&g_canard);
  if (capacity)
    *capacity = stats.capacity_blocks;
  if (usage)
    *usage = stats.current_usage_blocks;
  if (peak)
    *peak = stats.peak_usage_blocks;
  if (seq_errors)
    *seq_errors = g_seq_errors;
}

// Process a Cannelloni packet containing multiple CAN/CAN-FD frames
FFI_EXPORT void stork_canard_process_packet(const uint8_t *data,
                                            uint32_t data_len,
  ...
  if (version != 2 || op_code != 0) {
    stork_log("stork_canard: Ignoring non-DATA packet (expected version=2, "
              "op_code=0)\n");
    return;
  }
  ...
  if (g_last_seq_no != -1) {
    ...
    if (seq_no != expected_seq) {
      ...
      g_seq_errors += missed;
      stork_log("stork_canard: Sequence error detected! Expected %d, got %d. "
                "Missed packets: %d (Total errors: %u)\n",
  ...
  for (uint16_t i = 0; i < count; i++) {
    if (offset + 5 > data_len) {
      stork_log("stork_canard: Breaking: frame %d header out of bounds "
                "(offset=%d, len=%d)\n",
                i, offset, data_len);
      break;
    }
    ...
    if (offset + dlc > data_len) {
      stork_log("stork_canard: Breaking: frame %d data out of bounds (dlc=%d, "
                "offset=%d, len=%d)\n",
...
FFI_EXPORT int16_t stork_canard_broadcast(uint64_t data_type_signature,
                                          uint16_t data_type_id,
  ...
  int16_t res =
      canardBroadcast(&g_canard, data_type_signature, data_type_id,
                      inout_transfer_id, priority, payload, payload_len);
  return res;
}

FFI_EXPORT int32_t stork_canard_generate_tx_packet(uint8_t *out_buffer,
                                                   uint32_t max_len) {
...
FFI_EXPORT int16_t stork_canard_respond(
    uint64_t data_type_signature, uint16_t data_type_id, uint8_t transfer_id,
  ...
  int16_t res = canardRequestOrRespond(
      &g_canard, destination_node_id, data_type_signature, data_type_id, &tid,
      priority, CanardResponse, payload, payload_len);
  stork_log("stork_canard: Respond message type ID: %d, len: %d, priority: %d, "
            "res: %d\n",
