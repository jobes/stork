...
### 2. State Management
The offline map system is managed by the `OfflineMapsNotifier` (using Riverpod). It tracks:
...
### 3. Data Storage
All offline data is persisted in a local SQLite database managed by `DatabaseService`.
...
### Download Engine (`TileDownloadService`)
...
3. **Local Storage**: These features are parsed and stored in the local database, allowing the map to display interactive aeronautical details even when offline.
...
## Transparent Tile Serving (`MapAssetsServer`)
