...
void main() {
  ...
  test('Computes correct time-weighted averages under delta-compression', () async {
    ...
    final entries = [
      ...
      TelemetryEntry(
        ...
        isSnapshot: false,
        data: {'gps_altitude': 1010.0, 'engine_rpm': 2100.0},
      ),
      TelemetryEntry(
        flightUuid: flightUuid,
  ...
  test(
    'Computes correct distance and max distance from takeoff under delta-compression',
    () async {
      ...
      final entries = [
        ...
        TelemetryEntry(
          ...
          timestamp: startTime.add(const Duration(seconds: 3)),
          isSnapshot: false,
          data: {'latitude': 48.2, 'longitude': 17.2},
        ),
      ];
  ...
  test(
    'recoverUnfinishedFlights keeps flight retryable if stats calculation fails',
    () async {
      final tempDir = Directory.systemTemp.createTempSync(
        'blackbox_recovery_test_',
      );
