...
void main() {
  ...
  group('PureTrackAuthService Tests', () {
    ...
    test('init loads token from storage if present', () async {
      when(() => mockStorage.read(key: 'puretrack_access_token'))
          .thenAnswer((_) async => 'mock_token_123');
      when(() => mockStorage.read(key: 'puretrack_username'))
          .thenAnswer((_) async => 'pilot@example.com');

      final token = await authService.init();

      expect(token, equals('mock_token_123'));
      expect(authService.currentState, equals(PureTrackAuthState.authenticated));
    ...
    test('init sets unauthenticated if token not present', () async {
      ...
      expect(token, isNull);
      expect(authService.currentState, equals(PureTrackAuthState.unauthenticated));
    });
    ...
    test('login persists token on 200 response', () async {
      ...
      final result = await authService.login(username: 'pilot@example.com', password: 'secretpassword');

      expect(result.isSuccess, isTrue);
      expect(result.token, equals('valid_jwt_token'));
      expect(result.isPro, isTrue);
      expect(authService.currentState, equals(PureTrackAuthState.authenticated));
      verify(() => mockStorage.write(key: 'puretrack_access_token', value: 'valid_jwt_token')).called(1);
    });
    ...
    test('login handles 401 failure gracefully', () async {
      ...
      final result = await authService.login(username: 'pilot@example.com', password: 'wrongpassword');
    ...
    test('invalidateToken clears storage and sets state to tokenInvalid', () async {
      when(() => mockStorage.delete(key: 'puretrack_access_token'))
          .thenAnswer((_) async => {});

      await authService.invalidateToken();

      expect(authService.currentToken, isNull);
      expect(authService.currentState, equals(PureTrackAuthState.tokenInvalid));
      verify(() => mockStorage.delete(key: 'puretrack_access_token')).called(1);
    });

    test('logout clears secure storage and sets state to unauthenticated', () async {
