...
void main() {
  group('AircraftRepository Tests', () {
    ...
    test('getAircrafts parses a valid JSON list of aircrafts correctly', () async {
      final repository = AircraftRepository(prefs);
      const originalAircrafts = [
        Aircraft(id: '1', name: 'Cessna 172', initialFlightHours: 15.5, initialFlights: 10),
        Aircraft(id: '2', name: 'Piper Archer', initialFlightHours: 2.0, initialFlights: 1),
      ];
    ...
    test('getAircrafts throws FormatException when JSON is malformed', () async {
      final repository = AircraftRepository(prefs);
    ...
    test('getAircrafts throws TypeError when JSON structure does not match expected schema', () async {
      final repository = AircraftRepository(prefs);
      // "initialFlights" has invalid type (string instead of int)
      await prefs.setString('app_aircrafts', '[{"id": "1", "name": "Cessna", "initialFlights": "ten"}]');
    ...
    test('getAircraft, saveAircraft and deleteAircraft work correctly', () async {
      final repository = AircraftRepository(prefs);
