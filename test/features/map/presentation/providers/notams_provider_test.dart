  ...
  group('Notams Provider Tests', () {
    ...
    setUp(() async {
      ...
      await FirUtils.initialize(
        rawJson: File('assets/geojson/fir.geojson').readAsStringSync(),
      );
    });
    ...
    test('Loads NOTAMs on initial GPS fix and only updates FIR on route changes', () {
      fakeAsync((async) {
        ...
        container
            ...
            .updateGPS(latitude: 48.17, longitude: 17.17, groundSpeed: 0.0);
        ...
        expect(
          mockRepository.queriedFirsLogs,
          equals([
            ['LZBB'],
          ]),
        );
        ...
        container
            .read(telemetryProvider.notifier)
            .updateGPS(latitude: 50.0, longitude: 15.0);
        async.elapse(const Duration(milliseconds: 100));
        ...
        final flatQueries = mockRepository.queriedFirsLogs
            .expand((x) => x)
            .toList();
    ...
    test(
      'Does not reload NOTAMs when point is removed due to auto-advance, but reloads on manual removal',
      () {
        fakeAsync((async) {
          mockRepository.mockNotamsToReturn = [testNotam];
