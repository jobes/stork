...
void main() {
  ...
  group('Navigation Auto-Advance Tests', () {
    ...
    test('does not auto-advance when time to next point is > 60 seconds', () async {
      final notifier = container.read(navigationProvider.notifier);

      // Point 1 is at 48.01, 17.0 (~1110 meters away)
      const point1 = NavigationPoint(latitude: 48.01, longitude: 17.0, name: 'Point 1');
      const point2 = NavigationPoint(latitude: 48.05, longitude: 17.0, name: 'Point 2');
    ...
    test('auto-advances when time to next point is <= 60 seconds (using settings speed)', () async {
      final notifier = container.read(navigationProvider.notifier);

      // Point 1 is at 48.004, 17.0 (~444 meters away)
      const point1 = NavigationPoint(latitude: 48.004, longitude: 17.0, name: 'Point 1');
      const point2 = NavigationPoint(latitude: 48.05, longitude: 17.0, name: 'Point 2');
      await notifier.addPoint(point1);
      await notifier.addPoint(point2);
    ...
    test('auto-advances using ground speed when flying', () async {
      ...
      const point1 = NavigationPoint(latitude: 48.02, longitude: 17.0, name: 'Point 1');
      const point2 = NavigationPoint(latitude: 48.05, longitude: 17.0, name: 'Point 2');
      await notifier.addPoint(point1);
      await notifier.addPoint(point2);
    ...
    test('auto-advances multiple points at once when all are under 60 seconds threshold', () async {
      final notifier = container.read(navigationProvider.notifier);
      ...
      const point1 = NavigationPoint(latitude: 48.002, longitude: 17.0, name: 'Point 1');
      const point2 = NavigationPoint(latitude: 48.004, longitude: 17.0, name: 'Point 2');
      const point3 = NavigationPoint(latitude: 48.050, longitude: 17.0, name: 'Point 3');
