...
void main() {
  ...
  testWidgets('shows warning when current location is unknown', (WidgetTester tester) async {
    SharedPreferences.setMockInitialValues({});
    await tester.pumpWidget(buildTestWidget());
    await tester.pumpAndSettle();

    expect(find.text('Navigation requires current location.'), findsOneWidget);
  });

  testWidgets('uses settings expected flight speed when not flying', (WidgetTester tester) async {
    SharedPreferences.setMockInitialValues({});
    await tester.pumpWidget(buildTestWidget());
    await tester.pumpAndSettle();
    ...
    expect(find.textContaining('Expected flight speed: 100 kph'), findsOneWidget);
  });

  testWidgets('uses GPS ground speed and shows Ground Speed label when flying', (WidgetTester tester) async {
    SharedPreferences.setMockInitialValues({});
    await tester.pumpWidget(buildTestWidget());
    await tester.pumpAndSettle();
    ...
    providerContainer.read(telemetryProvider.notifier).updateGPS(
      latitude: 48.0,
      longitude: 17.0,
      groundSpeed: 50.0, // 50.0 m/s is > threshold (2.77 m/s) -> isFlying = true
    );
    await tester.pumpAndSettle();
